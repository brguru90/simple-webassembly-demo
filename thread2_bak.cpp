...
extern "C" void threadCompleteCallback(int);
...
private:
    ...
    static void *monitor_threads(void *th)
    {
        ...
        threaddata[0].callback(sum);
        pthread_exit(NULL);
    }

public:
    int sum = 0;
    myThreadClass(f_int_t callback)
    {
        threaddata[0].callback = callback;
        start_thread();
    }
};
...
extern "C"
{
    int test(void(*callback)(int))
    {
        myThreadClass *t = new myThreadClass(callback);
...
void main_call_back(int x)
{
    cout <<"In callback "<< x << endl;
}
int main()
{
    cout << "Return to main" << test(main_call_back) << endl;
    return 0;
}
